...
def add_embd_triplet_losses_labeled(model, label):
    ...
    if(cfg.TRAIN.HUBNESS):
        ...
        model.net.Reshape(['xp_yall_probT' + suffix], ['xp_yall_probT_reshaped'+suffix, 'xp_yall_probT_old_shape'+suffix], shape=(0,1, -1,1))
        xp_yall_probT_average_reshape_suffix =  model.net.AveragePool(['xp_yall_probT_reshaped'+suffix], ['xp_yall_probT_average_reshape' + suffix],  global_pooling=True )
        ...
        hubness_dist_suffix = model.net.Sub(['xp_yall_probT_average_reshape' + suffix, 'hubness_blob' + suffix], 'hubness_dist' + suffix, broadcast=1)
        hubness_dist_suffix_sqr = model.net.Sqr(['hubness_dist' + suffix], ['hubness_dist_sqr' + suffix])
        loss_hubness= hubness_dist_suffix_sqr.AveragedLoss([], ['loss_hubness' + suffix])
